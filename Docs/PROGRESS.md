...
[Reliability Under Loss](../Game Testing/Reliability.html) went 6 of 6: ten minutes at 5% induced loss
...
[L6](../Game Testing/Reliability.html) also closed [NET-24](ISSUE-REGISTER.md): the code read was
...
[Thumper-Defence](../Game Testing/Thumper-Defence.html) went 6 of 6 in one sitting. The milestone plan
...
but never resent, and [L6](../Game Testing/Reliability.html) is written to say whether that reading was
right.
...
it. [C1–C7](../Game Testing/Persistence.html) are the check and none of them have run.
...
them, and [Session Setup](../Game Testing/Session-Setup.html) had already recorded the consequence —
...
[test queue](../Game Testing/index.html). **M8 turned out to be shaped the same way and was built the same
...
[L1](../Game Testing/Reliability.html) itself.

**M4: where you thump matters is done, closed 2026-08-14 by
[S1–S6](../Game Testing/Thump-Placement.html) passing in full.** The exit condition was "thump a rich
...
[S1–S6](../Game Testing/Thump-Placement.html) were the check, and the ranges predicted for them
...
**M5: killing something pays is done, closed 2026-08-13 by [K1](../Game Testing/Kill-Rewards.html).** A
...
[K1–K5](../Game Testing/Kill-Rewards.html), because an entry judged on two kills reports a defect that
isn't there.
...
[I1](../Game Testing/Inventory.html) is stuck on. They are rolled, logged and dropped. That is the cut
...
[G2](../Game Testing/Resource-Payout.html).** A thumper called down at a player's feet ran its full
...
queue's usual ratio. **M2 closed the same day**, by [N14–N16](../Game Testing/NPC-Combat.html): zone 448
...
[I1](../Game Testing/Inventory.html)'s problem rather than M3's — holding the milestone open on it would
...
[I1](../Game Testing/Inventory.html) is in the item struct rather than in partial updates as a
mechanism.
...
[N1–N7](../Game Testing/NPC-Combat.html) ran on 2026-08-12 and all pass: an NPC notices you, turns to
...
**[NPC Combat](../Game Testing/NPC-Combat.html) went 13 of 13 that day**, and finished at 16 of 16 once
...
`TempSpawnTestEntities` came out with it. [N14–N16](../Game Testing/NPC-Combat.html) are the entries,
...
E6](../Game Testing/Environment.html) passed the same night: drowning reproduces retail effect 789's
...
- [x] Pay out on thumper completion, hardcoded per beacon — 200 crystite from grant 50329, seen in
  game by [G2](../Game Testing/Resource-Payout.html) on 2026-08-13
...
[S1–S6](../Game Testing/Thump-Placement.html) 6 of 6 passed
...
- [x] Pay the killer the resources that dropped — `KillRewardSim` on `CharacterDiedEvent`, crystite
  and the melded resources, seen by [K1](../Game Testing/Kill-Rewards.html) on 2026-08-13
...
[C1–C7](../Game Testing/Persistence.html) 0 of 7 run
...
[L1–L6](../Game Testing/Reliability.html) 0 of 6 run
...
- [ ] Test under induced packet loss — [L1](../Game Testing/Reliability.html) is the exit condition and
...
- **M2: NPCs that fight back** — COMPLETE 2026-08-13, on [N14–N16](../Game Testing/NPC-Combat.html)
- **M3: Resources come out of the ground** — COMPLETE 2026-08-13, on
  [G2](../Game Testing/Resource-Payout.html). Closed on a calldown made by an admin command, because the
  retail route needs an item and item delivery is I1's problem; G3 reopens the question when it is
  fixed
- **M5: Killing something pays** — COMPLETE 2026-08-13, on [K1](../Game Testing/Kill-Rewards.html).
...
- **The rest of environmental damage** — the base case is built, not deferred: drowning and melding
  ...
  [E1–E7](../Game Testing/Environment.html). What is still out of scope is everything around it —
